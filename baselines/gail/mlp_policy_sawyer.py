...
class MlpPolicy(object):
    ...
    def _init(self, ob_space, ac_space, hid_size, num_hid_layers, gaussian_fixed_var=True):
        ...
        ac = tf.clip_by_value( U.switch(stochastic, self.pd.sample(), self.pd.mode()) , clip_value_min = -1, clip_value_max = 1) # clip here instead
        self.ac = ac # clip between -1 and 1 for the sawyer robot
        self._act = U.function([stochastic, ob], [ac , self.vpred]) #clip

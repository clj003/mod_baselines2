...
def traj_segment_generator(pi, last_ob, last_jpos, run_reach, policy_func, env, reward_giver, horizon, stochastic):

    # print("trpo_mpi make sure horizon exists: ", horizon) # Verify that horizon exists
    ...
    logger.log("running reach")
    
    ob_space = env.observation_space
    ac_space = env.action_space

    # ------ For the reach loading -----
    env.set_robot_joint_positions(last_jpos) # load in from the one run
    ob = last_ob

    #time.sleep(5)

    #logger.log("finish running reach")

    #logger.log("ob is called to reset environment here")
    ...
    num_steps = 0 # this is new thing to take into account time steps
    ...
    while True:
        #ob = env._flatten_obs(env._get_observation()) # Over here we make sure the observation is the one we want because downstream reset for each iteration, for specific sawyer lift env in robosuite
        prevac = ac
        ...
        ob, true_rew, new, _ = env.step(ac) # I think sawyerlift has sparse reward

        #env.render() # render in trpo_mpi traj gen to see if action is happening

        #time.sleep(5) # turns out the running in the environment does not work out as expected 
        ...
        # Episode never ends by the default setting so the returns should always be empty
        #if new: # change from just if new, to also include if horizon is met
        if (new or cur_ep_len > horizon): # change from just if new, to also include if horizon is met
            logger.log("resetting episode")

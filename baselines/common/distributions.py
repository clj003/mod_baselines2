...
class BernoulliPd(Pd):
    ...
    def neglogp(self, x):
        return tf.reduce_sum(tf.nn.sigmoid_cross_entropy_with_logits(logits=self.logits, labels=tf.cast(x, dtype=float32)), axis=-1) # change to_float
    def kl(self, other):
    ...
    def sample(self):
        ...
        return tf.cast(math_ops.less(u, self.ps), dtype=float32 ) # change to_float to cast
    @classmethod
    def fromflat(cls, flat):
        return cls(flat)

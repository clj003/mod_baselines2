...
import numpy as np
import re
from baselines.old_acktr.kfac_utils import *
...
class KfacOptimizer():
    ...
    def compute_stats(self, loss_sampled, var_list=None):
        ...
        # Print the sampled gradients
        print("gradient sampled line 300 kfac len: ", len(gs) )
        print("gradient sampled line 300 kfac: ", gs )
    ...
    def getKfacPrecondUpdates(self, gradlist, varlist):
        ...
        for g, var in zip(gradlist, varlist):
            ...
            # Euclidean norm of new grad needs to be done smth for print
            
            tf.Print(grad, [tf.sqrt(tf.reduce_sum(tf.pow(grad, 2)))], "Euclidean norm of new grad")
            #print("Euclidean norm of new grad: " )
            #tf.print([tf.sqrt(tf.reduce_sum(tf.pow(grad, 2)))])

...
def dense(x, size, name, weight_init=None, bias_init=0, weight_loss_dict=None, reuse=None):
    with tf.variable_scope(name, reuse=reuse):
        assert (len(tf.compat.v1.get_variable_scope().name.split('/')) == 2) # changed from 2 to 1 since initalized differently
        ...
        if weight_loss_dict is not None:
            ...
            tf.compat.v1.add_to_collection(tf.get_variable_scope().name.split('/')[0] + '_' + 'losses', weight_decay)
